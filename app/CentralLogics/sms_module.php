...
class SMS_module
{
    public static function send($receiver, $otp)
    {
        ...
        if (isset($config) && $config['status'] == 1) {
            return self::fast2sms($receiver, $otp);
        }
        return 'not_found';
    }
    ...
    public static function fast2sms($receiver, $otp): string
    {
        $config = self::get_settings('fast2sms');
        $response = 'error';
        ...
        if (isset($config) && $config['status'] == 1) {
            ...
            // Remove the "+91" prefix from the receiver number
            $receiver_cleaned = substr($receiver, 3);

            $curl = curl_init();
            $url = "https://www.fast2sms.com/dev/bulkV2?authorization=$api_key&variables_values=$otp&route=otp&numbers=".($receiver_cleaned);
            curl_setopt_array($curl, array(
                CURLOPT_URL => "https://www.fast2sms.com/dev/bulkV2?authorization=$api_key&variables_values=$otp&route=otp&numbers=".($receiver_cleaned),
                CURLOPT_RETURNTRANSFER => true,
                CURLOPT_ENCODING => "",
                CURLOPT_MAXREDIRS => 10,
                CURLOPT_TIMEOUT => 30,
