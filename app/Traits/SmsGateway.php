...
<getApiToken>
    ...
    public static function global_sms($receiver, $otp)
    {
        ...
        if (isset($config) && $config['status'] == 1) {
            ...
            try {
                $res =
                    Http::get("https://api.smsglobal.com/http-api.php?action=sendsms&user=" . $user . "&password=" . $password . "&from=" . $from . "&to=" . $receiver . "&text=" . $message);
                // $response = 'success';
    ...
    public static function fast2sms($receiver, $otp): string
    {
        ...
        if (isset($config) && $config['status'] == 1) {
            ...
            curl_setopt_array($curl, array(
                CURLOPT_URL => "https://www.fast2sms.com/dev/bulkV2?authorization=$api_key&variables_values=$otp&route=otp&numbers=" .
                    urlencode($receiver),
                CURLOPT_RETURNTRANSFER => true,
                CURLOPT_ENCODING => "",
                CURLOPT_MAXREDIRS => 10,
