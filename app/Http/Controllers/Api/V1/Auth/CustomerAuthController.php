...
class CustomerAuthController extends Controller
{
    ...
    public function login_mobile(Request $request)
    {
        ...
        // Prepend the country code to the phone number if necessary
        ...
        // if (!$user->status) {
        //     return response()->json(['error' => translate('messages.your_account_is_blocked')], 403);
        // }

        // Check if the application is not in demo mode and the user is logging in via web
        if (env('APP_MODE') != 'demo' && $user->login_medium === 'web') {
            ...
            if ($verification_data && Carbon::parse($verification_data->updated_at)->diffInSeconds() < $otp_interval_time) {
                $time = $otp_interval_time - Carbon::parse($verification_data->updated_at)->diffInSeconds();
                return response()->json(['error' => translate('messages.please_try_again_after_') . $time . ' ' . translate('messages.seconds')], 405);
            }
            ...
            if ($response != 'success') {
                return response()->json(['error' => translate('messages.failed_to_send_sms')], 405);
            }

            // Return the access token and phone verification status
            return response()->json(['token' => $user->createToken('RestaurantCustomerAuth')->accessToken, 'status' => true], 200);
        }

        // Return an error response if the user is not found or inactive
        return response()->json(['error' => 'Invalid phone number or user inactive'], 403);
    }
