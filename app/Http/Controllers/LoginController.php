...
class LoginController extends Controller
{
    ...
    public function reset_password(Request $request)
    {
        ...
        if (!$data || Carbon::parse($data->created_at)->diffInMinutes(Carbon::now()) >= 5000) {
        ...
        if ($data->created_by == 'admin') {
            ...
            if ($response != 'error') {
                ...
                // return view('auth.reset-password', compact('token','admin','site_direction','locale'));  because directly redirect change password page without verify otp
            }
            Toastr::error("Otp limit cross");
            return back();
        } else {
    ...
    {
        $data = DB::table('password_resets')->where(['token' => $request['token']])->first();
        if (!$data || Carbon::parse($data->created_at)->diffInMinutes(Carbon::now()) >= 500) {
