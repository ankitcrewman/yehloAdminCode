...
class SMSModuleController extends Controller
{
    public function sms_index()
    {
        ...
        $data_values=  Setting::where('settings_type','sms_config')->whereIn('key_name', ['twilio','nexmo','2factor','msg91','alphanet_sms','fast2sms'])->get() ?? [];
    ...
    if ($request['status'] == 1) {
        foreach (['twilio','nexmo','2factor','msg91','alphanet_sms','fast2sms'] as $gateway) {
            if ($module != $gateway) {
