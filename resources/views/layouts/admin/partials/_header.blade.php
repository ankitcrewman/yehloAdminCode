                    ...
                    <li class="nav-item __nav-item">
                        <a href="javascript:void(0)" class="__nav-link module--nav-icon" id="tourb-0">
                            @if ($mod)
                                ...
                                @elseif ($mod->module_type != 'services')
                                    <img src="{{ \App\CentralLogics\Helpers::onerror_image_helper($mod->icon, asset('storage/app/public/module/') . '/' . $mod->icon, asset('/public/assets/admin/img/new-img/module-icon.svg'), 'module/') }}"
                                        class="onerror-image"
                                        data-onerror-image="{{ asset('/public/assets/admin/img/new-img/module-icon.svg') }}"
                                        width="20px" alt="public/img">
                                ...
                                @endif
                                <span
                                    class="text-white">{{ $mod->module_type != 'services' ? $mod->module_name : translate('modules') }}</span>
                                <img src="{{ asset('/public/assets/admin/img/new-img/angle-white.svg') }}"
                                    class="d-none d-lg-block ml-xl-2" alt="public/img">
                            @else
                                <span class="text-white">{{ translate('Modules') }}</span>
                            @endif
                        </a>
                    ...
                    <li class="nav-item __nav-item">
                        <a href="javascript:void(0)" class="__nav-link module--nav-icon" id="tourb-0">
                            @if ($mod_service)
                                ...
                                @endif
                                <span
                                    class="text-white">{{ $mod_service->module_type == 'services' ? $mod_service->module_name : translate('services') }}</span>
                                <img src="{{ asset('/public/assets/admin/img/new-img/angle-white.svg') }}"
                                    class="d-none d-lg-block ml-xl-2" alt="public/img">
                            @else
                                <span class="text-white">{{ translate('Services Module') }}</span>
                            @endif
                        </a>
                        <div class="__nav-module style-2" id="tourb-1">
                            @php(
    $modules_service = \App\Models\Module::when(auth('admin')->user()->zone_id, function ($query) {
        $query->whereHas('zones', function ($query) {
            $query->where('zone_id', auth('admin')->user()->zone_id);
        });
    })->where('module_type', 'services')->Active()->get()
                                ...
                                <div class="__nav-module-body">
                                    <div class="__nav-module-items">
                                        @foreach ($modules_service as $module_servive)
                                            <a href="javascript:" data-module-id="{{ $module_servive->id }}"
                                                data-url="{{ route('admin.dashboard') }}" data-filter="module_id"
